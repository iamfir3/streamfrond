      ...
      <div className="flex-auto pl-[38px] pr-[60px]" style={{ background: "linear-gradient(113.82deg, #000000 0%, #1A1A1A 96.93%)" }}>
        <div className="h-[110px] flex items-center justify-between">
            ...
            <div className={`absolute w-full bg-bg1 h-[500px] overflow-y-auto left-0 rounded-[12px] z-[13] top-[70px] cursor-pointer ${isSearching === false ? "hidden" : ""}`}
              ...
              <ReactPerfectScrollbar>
                ...
                {searchedSong?.map((song, i) => {
                  return <div key={i} className="flex justify-between pl-[20px] pr-[15px] py-[15px]"
                    onMouseDown={(e) => { e.stopPropagation(); handlePlaySong(song?.id) }}>
                    <div>
          ...
          <div className="flex items-center gap-[30px]">
            ...
            {isLoggedIn === false ? <Link to='/login' className="bg-[#262626] rounded-[40px] px-[20px] py-[10px]">
              <p>Login</p>
            </Link> :
              <div className="bg-[#262626] rounded-[40px] px-[20px] py-[10px] flex gap-[12px] items-center relative">
                <img src={avatar}></img>
                <label htmlFor="show">
                  <p className="font-[600] text-[14px] text-white">{localStorage.getItem("username")}</p>
